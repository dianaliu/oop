...
public class CPPPrinter extends Visitor {
    ...
    public void visitTranslationUnits(GNode n) {
	// Reset the state.
	isDeclaration  = false;
	isLongDecl     = false;
	isStatement    = false;
    ...
    **/
    ...
    // used to visit our combined tree
    ...
    // for combined
    // Need to dedupe?
    public void visitBothPrimitiveTYPES(GNode n) {
	printer.indent().pln("// Begin visitBothPrimitiveTYPES");
	...
	printer.indent().pln("// End visitBothPrimitiveTYPES");
    ...
    // for combined
    // Need to dedupe?
    public void visitbothArrayTemplates(GNode n) {
	printer.indent().pln("// Begin visitbothArrayTemplates");
	...
	printer.indent().pln("// End visitbothArrayTemplates");
    ...
    **/ 
    ...
    // for combined
    public void visitClassDeclarations(GNode n) {
	...
	cn = n.getNode(1).getNode(0).getString(1);
	...
	/**
	// Should be done immediately after your own 
	...
	// ? Where should the empty Constructor go? Does Grimm use it?
	...
	// hacky, can I put the destructor and other declarations here?	

	// Should be done immediately after your own 
	// Add destructor to namespace java::lang

	printer.indent().pln("// The destructor");
	printer.indent().p("void __").p(globalCN).p("::__delete(__").p(globalCN);
	printer.p("* __this) {").pln();
	printer.incr();
	printer.indent().pln("delete __this;");
	printer.decr();
	...
	printer.indent().p("__").p(globalCN).p("_VT __").p(globalCN);
	printer.p("::__vtable;").pln();
	...
	printer.indent().p("__").p(vtClass).p("_VT()");
