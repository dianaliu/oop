...
public class LeafTransplant extends Visitor implements CPPUtil {
    ...
    public LeafTransplant(ClassLayoutParser clp, GNode javaAST) { 
		...
		//automatically dispatch the translator after everything is initialized:
    ...
    GNode buildHeaderForClass() {
		...
		{   
			// 1. Set objectTree.getNode(1) = dataLayout node
			...
			objectTree.add(0, dataLayout);
			
			// 2. Set objectTree.getNode(2) = vtableLayout
			...
			objectTree.add(1, vtableLayout);
			
			// 3. Set objectTree.getNode(0) = Typedef Declaration node
    ...
    GNode functionDefForMethDecl(GNode n) {
		...
		GNode fncDef = GNode.create(kFuncDef);
		{
			...
			{
				GNode simpDecl = (GNode)GNode.create(kSimpDecl).add( "__" + thisClassName + "::" +  n.get(3));  //method name
    ...
    public void visitClassDeclaration(GNode n) {
		thisClassName = n.get( 1 ).toString( );  //set the global variable for the current class name
		translatedTree.add( buildHeaderForClass( ) );	//build and add a header for the current class name
		
		// ----- Begin creating the data layout
		
		ArrayList dataLayoutList = classParser.getDataLayout(thisClassName);
		//index 0 = __vptr
    ...
    public void visitMethodDeclaration(GNode n) {
		//the method should already have been added to the vtable, so that does not need to be done
		
		//adding the actual method implementation (will go in the .cc file, includes the full method header and body):
